...
Page({
  ...
  build() {
    ...
    if (!isArrayEmpty(initH) || !isArrayEmpty(initM[0])) {
      console.log("PASS")
      for (let p = 0; p < initH.length; p++) {
        if (!isArrayEmpty(initH[p]) || !isArrayEmpty(initM[p])) {
          console.log("TEST")
          let num = initH[p][0]
          console.log("N: " + num)
          if (num >= 10) {
            console.log("TRUE")
            initT[p][0] = String(num).substring(0, 1)
            initT[p][1] = String(num).substring(1, 2)
            console.log("PASS T")
          } else {
            console.log("FALSE")
            initT[p][0] = 0
            initT[p][1] = String(num).substring(0, 1)
            console.log("PASS F")
          }
          let numT = initM[p][0]
          initT[p][2] = String(numT).substring(0, 1)
          initT[p][3] = String(numT).substring(1, 2)
          console.log("END")
          sTimeHour = initH
          sTimeMin = initM
        }else{
